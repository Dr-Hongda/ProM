...
public class PromTestFramework {
	...
	public Object main(CommandLineArgumentList commandlineArguments) throws Throwable {
		...
		if (commandlineArguments.size() != 2) {
			throw new Exception(
					"Error. The ProM Test Framework requires 2 arguments: (1) location of classes that contain tests, (2) location of test files");
		}
		...
		for (Method test : testMethods) {
			...
			try {
				...
				if (!result.equals(expected)) {
					...
					failedTest.add(new PromTestException.ResultMismatch(test, expected, result));
				}
			} catch (Throwable e) {
				// test crashed, store exception for reporting
				errorTest.add(new PromTestException.WrappedException(test, e));
			}
		}
	...
	private String loadClassFromFile(URLClassLoader loader, URL url, String classFilename) {
		...
		return loadClass(
				loader,
				url,
				classFilename.substring(0, classFilename.length() - PluginManager.CLASS_EXTENSION.length())
	...
	private String loadClass(URLClassLoader loader, URL url, String className) {
		...
		try {
			...
			/*
			 * // Check if plugin annotation is present if
			 * (pluginClass.isAnnotationPresent(Plugin.class) &&
			 * isGoodPlugin(pluginClass)) { PluginDescriptorImpl pl = new
			 * PluginDescriptorImpl(pluginClass, pluginContextType);
			 * addPlugin(pl); }
			 */
	...
	private boolean isGoodTest(Method method) {
		...
		if ((method.getModifiers() & Modifier.STATIC) == 0) {
			if (Boot.VERBOSE != Level.NONE) {
				System.err.println("Test " + method.toString() + " could not be loaded. " + "Test must be static.");
			}
			return false;
		}
	...
	private static boolean testResultFromFile(Method method) {
		assert (method.isAnnotationPresent(TestMethod.class));
		return (method.getAnnotation(TestMethod.class).filename() != null)
	 ...
	 * @param method
	 * @return <code>true</code> iff the method is annotated with
	 *         {@link TestMethod#output()}. Then the result of the test will be
	 *         compared to the specified string.
	 */
	private static boolean testResultFromOutputAnnotation(Method method) {
		assert (method.isAnnotationPresent(TestMethod.class));
		return (method.getAnnotation(TestMethod.class).output() != null)
