...
/**
 * Script interpretor for ProM. The class provides a wrapper around the Java BSH
 * {@link Interpreter} to initialize all available plugins from the .ProM user
 * directory in an {@link Interpreter}. This interpreter can the be used to
 * interpret Java code at run-time against the plugins, which allows for
 * scripted execution of ProM plugins.
 *
...
public class ScriptExecutor {
	...
	/**
	 * Iterate over all registered plugins, find their declared methods and try
	 * to create an invocation context for each. Return the list of plugins that
	 * succeeded, and save the list of methods that failed.
	 *
	 * @return
	 */
				...
				for (int j = 0; j < plugin.getParameterTypes().size(); j++) {
					...
					if (!foundSignatures.contains(signature)) {
						...
						if (signature.getReturnTypes().size() > 1) {
							...
							init.append("    for (int i = 0; i < result.length; i++) { result[i] = context.getResult().getResult(i); }"
									+ nl);
	...
	private Signature getSignature(PluginDescriptor plugin, int index) {
		...
		return new Signature(plugin.getReturnTypes(), StringUtils.getJavaIdentifier(name),
				plugin.getParameterTypes(index));
	}
