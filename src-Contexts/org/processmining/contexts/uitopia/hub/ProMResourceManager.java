...
public class ProMResourceManager extends UpdateSignaller implements ResourceManager<ProMResource<?>>,
	...
	private boolean importResourceNotInEDT(final PluginParameterBinding binding, final File... files) {
		...
		/*
		 * Synchronize on the provided object manager to prevent multiple
		 * imports talking at the same time to this manager.
		 */
		synchronized (context.getProvidedObjectManager()) {
			for (File f : files) {
				...
				try {
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							JOptionPane.showMessageDialog(context.getUI(), "<html>Error with import of " + files
									+ ":<br>" + e.getMessage() + "</html>", "Import failed", JOptionPane.ERROR_MESSAGE);
						}
					});
