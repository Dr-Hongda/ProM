...
/**
 * The base class for unit tests in ProM. A class the extends this class is a
 * JUnit test suite that gets automatically enriched with two kinds of tests.
 *
 * 1) Inclass Method Tests: these are tests generated from all methods in the
 * plugin code that are annotated with {@link TestMethod} and the corresponding
 * attributes. Tests are generated from all classes stored in
 * {@value #defaultClassFileLocations}.
 *
 * 2) Automated test scripts: these are tests that run a sequence of scripted
 * tests that are stored in a specified location: {@link #defaultTestScriptDir}.
 *
 * The tests may access files/compare results to files stored in
 * {@link #defaultTestDir}.
 *
 * @author Dirk Fahland
 */
...
public class PromTest extends TestCase {
	...
	/**
	 * Default location of test script files inside {@link #defaultTestDir} that
	 * is used if the system property <code>test.testScriptRoot</code> is not
	 * set. By default, this location is {@value #defaultTestScriptDir}.
	 ...
	 * with {@link TestMethod} and which shall be run in the JUnit test.
	 * Overwrite this value if necessary. Default value {@value #defaultTestDir}
	 * .
	 */
